      ...
      if (botData.length === 1) {
        ...
        if (botData[0]?.client) {
          try {
            ...
            console.error(`Error getting client info for company ${companyId}:`, err);
          }
        }
        ...
        const statusArray = await Promise.all(botData.map(async (phone, index) => {
          ...
          if (phone?.client) {
            try {
              ...
              console.error(`Error getting client info for company ${companyId} phone ${index}:`, err);
            }
          }
...
async function syncContacts(client, companyId, phoneIndex = 0) {
  ...
  try {
    ...
    const totalChats = CHAT_LIMIT ? Math.min(chats.length, CHAT_LIMIT) : chats.length;
    let processedChats = 0;
    let failedChats = [];
    
    console.log(`Found ${chats.length} chats for company ${companyId}, phone ${phoneIndex}. Processing ${totalChats} chats.`);
        ...
        try {
          ...
          for (const message of messages) {
            try {
              ...
              console.error(`Error saving message ${message.id._serialized}:`, error);
            }
          }
    ...
    console.log(`Finished syncing contacts and messages for company ${companyId}, phone ${phoneIndex}`);
    ...
    console.error(`Error syncing contacts and messages for company ${companyId}, phone ${phoneIndex}:`, error);
    throw error;
  }
}
  ...
  const worker = new Worker(`scheduled-messages-${botId}`, async job => {
    if (job.name === 'send-message-batch') {
      if (!isWithinAllowedHours()) {
        console.log(`Bot ${botId} - Message delayed: Outside allowed hours (2 AM - 6 AM)`);
...
async function sendScheduledMessage(message) {
  ...
  if (!isWithinAllowedHours()) {
    console.log(`[Company ${companyId}] Message delayed: Outside allowed hours (2 AM - 6 AM)`);
    // Calculate delay until 6 AM
    const now = new Date();
    const nextAllowed = new Date();
    nextAllowed.setHours(6, 0, 0, 0);
    ...
    console.log(`[Company ${companyId}] Waiting ${Math.round(delay / 1000 / 60)} minutes until 6 AM`);
    ...
    const waitUntilNextDay = async () => {
      ...
      while (true) {
        ...
        try {
          ...
          const endpoint = message.mediaUrl ? 'image' : 
                          message.documentUrl ? 'document' : 'text';
...
app.get('/api/user/config', async (req, res) => {
  try {
    ...
    const user = await sqlDb.getRow('SELECT * FROM users WHERE email = $1', [userEmail]);
...
app.get('/api/companies/:companyId', async (req, res) => {
  try {
    const { companyId } = req.params;
    const company = await sqlDb.getRow('SELECT * FROM companies WHERE company_id = $1', [companyId]);
...
async function checkAndScheduleDailyReport() {
  try {
    ...
    try {
      ...
      console.log(`Found ${companies.length} companies to check for daily reports`);
        ...
        try {
          ...
          if (settings?.enabled) {
            ...
            if (!wasRunToday && 
              ...
              console.log(`[${companyId}] Daily report not sent yet today and it's past scheduled time. Sending now...`);
          ...
          console.error(`[${companyId}] Error setting up daily report:`, companyError);
...
async function getAllPhoneStatusesSql(companyId) {
  try {
    const query = 'SELECT phone_number, status, metadata FROM phone_status WHERE company_id = $1';
    const { rows } = await sqlDb.query(query, [companyId]);
    return rows;
  } catch (error) {
    console.error(`Error getting all phone statuses from SQL for ${companyId}:`, error);
    return [];
  }
}
...
async function fetchCompanyConfigSql(companyId) {
  try {
    ...
    const openaiTokenQuery = 'SELECT config_value FROM system_config WHERE config_key = $1';
    const openaiTokenResult = await sqlDb.query(openaiTokenQuery, ['openai_api_key']);
...
async function fetchEmployeesDataSql(companyId) {
  try {
    const query = 'SELECT id, employee_id, name, email, phone AS "phoneNumber", role FROM employees WHERE company_id = $1';
    const { rows } = await sqlDb.query(query, [companyId]);
    return rows;
  } catch (error) {
    console.error(`Error fetching employees data from SQL for company ${companyId}:`, error);
    return [];
  }
}
...
async function updateMonthlyAssignmentsSql(companyId, employeeName, incrementValue) {
  ...
  try {
    ...
    const employeeUpdateQuery = `
      ...
      RETURNING id;
    `;
    const employeeResult = await client.query(employeeUpdateQuery, [incrementValue, companyId, employeeName]);

    if (employeeResult.rows.length === 0) {
      throw new Error(`Employee '${employeeName}' not found for company '${companyId}'`);
    }
    ...
    `;
    await client.query(monthlyAssignmentUpsertQuery, [employeeId, companyId, currentMonthKey, incrementValue]);
...
app.post('/api/employees/update-monthly-assignments', async (req, res) => {
  try {
    ...
    res.json({ success: true, message: 'Monthly assignments updated successfully' });
  } catch (error) {
...
app.get('/api/companies/:companyId/scheduled-messages-summary', async (req, res) => {
  try {
    const { companyId } = req.params;
    ...
    res.status(500).json({ error: 'Failed to fetch scheduled messages summary' });
  }
});
...
app.get('/api/companies/:companyId/employee-stats/:employeeId', async (req, res) => {
  try {
  ...
  try {
    ...
    try {
        ...
        if (phoneIndex !== undefined) {
          ...
          if (botData[phoneIndex]?.client) {
            try {
              ...
              console.error(`Error destroying client for ${botName} phone ${phoneIndex}:`, error);
            }
          }
        } else {
          // Full bot reinitialization
          await Promise.all(botData.map(async (data) => {
            if (data?.client) {
              try {
                await data.client.destroy();
              } catch (error) {
                console.error(`Error destroying client for ${botName}:`, error);
              }
            }
          }));
      ...
      if (phoneIndex !== undefined) {
        ...
        const sessionDir = path.join(__dirname, '.wwebjs_auth', `session-${botName}${phoneCount > 1 ? `_phone${phoneIndex + 1}` : ''}`);
        try {
        ...
        for (let i = 0; i < phoneCount; i++) {
          const sessionDir = path.join(__dirname, '.wwebjs_auth', `session-${botName}${phoneCount > 1 ? `_phone${i + 1}` : ''}`);
          try {
            await fs.promises.rm(sessionDir, { recursive: true, force: true });
            console.log(`Deleted session directory: ${sessionDir}`);
            sessionsCleaned = true;
    ...
    res.json({ 
      ...
      message: sessionsCleaned ? 
        `${phoneIndex !== undefined ? `Phone ${phoneIndex + 1}` : 'Bot'} reinitialized successfully with clean session` : 
        `${phoneIndex !== undefined ? `Phone ${phoneIndex + 1}` : 'Bot'} reinitialized successfully`,
      phoneCount,
      phoneIndex,
      sessionsCleaned
    });
  ...
  app.post('/api/v2/messages/text/:companyId/:chatId', async (req, res) => {
    ...
    try {
      ...
      const messageData = {
        ...
        company_id: companyId,
        contact_id: chatId.split('@')[0],  // Changed to match sync code: contact.id.user
        thread_id: chatId,                 // Added to match sync code
        customer_phone: chatId.split('@')[0], // Changed to match sync code: contact.id.user
        content: message,
        message_type: type2,
        media_url: null,                   // Added to match sync code
        ...
        try {
          contactResult = await sqlDb.query(
            `INSERT INTO contacts (contact_id, phone, company_id, last_updated)
             VALUES ($1, $2, $3, CURRENT_TIMESTAMP)
             RETURNING *`,
            [chatId, phoneNumber, companyId] // phoneNumber is the receiver's number
          );
        } catch (err) {
          if (err.code === '23505') { // Unique violation
            contactResult = await sqlDb.query(
...
async function updatePhoneStatus(companyId, phoneIndex, status, details = {}) {
  try {
    ...
    console.log(`Updated status for ${companyId} Phone ${phoneIndex + 1} to ${status}`);
...
console.log(`Starting phone monitoring for ${botName} Phone ${phoneIndex + 1}`);
  const intervalId = setInterval(async () => {
    try {
      ...
      if (statusDoc.exists && statusDoc.data().status === 'initializing') {
        console.log(`${botName} Phone ${phoneIndex + 1} - Still initializing, running cleanup...`);
    ...
    try {
      ...
      client.on('error', async (error) => {
        ...
        try {
          ...
          console.log(`${botName} Phone ${phoneIndex + 1} - Error detected, attempting cleanup and reinitialization...`);
          
          const { spawn } = require('child_process');
    
      
        } catch (handlingError) {
          console.error(`${botName} Phone ${phoneIndex + 1} - Error handling client error:`, handlingError);
          try {
            await initializeBot(botName, 1, phoneIndex);
          } catch (lastError) {
            console.error(`${botName} Phone ${phoneIndex + 1} - Last resort reinitialization failed:`, lastError);
          }
        }
      });
        ...
        try {
          ...
          if (botData?.[phoneIndex]?.client) {
            try {
              ...
              if (browser) {
                await browser.close().catch(err => console.log('Browser close error:', err));
              }
              await botData[phoneIndex].client.destroy().catch(err => console.log('Client destroy error:', err));
            } catch (closeError) {
          ...
          console.log(`${botName} Phone ${phoneIndex + 1} - Running cleanup...`);
          const { spawn } = require('child_process');
        ...
        for (const { client } of botData) {
          if (client && typeof client.destroy === 'function') {
            clientShutdownPromises.push(
              client.destroy()
                .then(() => console.log(`WhatsApp client destroyed for bot ${botName}`))
                .catch(err => console.error(`Error destroying WhatsApp client for bot ${botName}:`, err))
            );
            ...
            if (client?.pupPage) {
              clientShutdownPromises.push(
                client.pupPage.browser().close()
                  ...
                  .catch(err => console.error(`Error closing browser for bot ${botName}:`, err))
              );
            }
          }
        }
    ...
    try {
      ...
      if (process.platform === 'win32') {
        ...
        try {
          ...
          console.warn(`Windows delete failed, trying fs.rm: ${winError.message}`);
...
app.get('/api/companies/:companyId/contacts', async (req, res) => {
  ...
  if (!email) {
    ...
    if (!userData) {
      return res.status(403).json({ error: 'Forbidden - User not authorized for this company' });
    }
...
function filterContactsByUserRole(contacts, userRole, userName) {
  ...
  if (userRole === '2') {
    return contacts.filter(contact => {
      ...
      const isAssignedToUser = contact.assignedTo && 
        contact.assignedTo.some(assignee => 
          assignee.toLowerCase() === userName.toLowerCase()
        );
      
      // Check if contact has user's name in tags
      const hasUserTag = contact.tags && 
