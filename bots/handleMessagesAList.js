...
async function sendScheduledAppointmentReminders(client) {
    const userRef = db.collection('user').doc('thealistmalaysia@gmail.com');
    const reminderCollectionRef = userRef.collection('scheduledAppointmentReminders');
    const now = admin.firestore.Timestamp.now();

    try {
        const scheduledReminders = await reminderCollectionRef
            .where('scheduledTime', '<=', now)
            .where('reminderSentStatus', '==', false)
            .get();

        
        for (const doc of scheduledReminders.docs) {
            const reminder = doc.data();
            const formattedNumberForWhatsApp = reminder.phoneNumber.slice(1) + '@c.us';

            try {
                const sent = await client.sendMessage(formattedNumberForWhatsApp, reminder.message);

                const messageData = {
                    chat_id: reminder.phoneNumber + '@c.us',
                ...
                const contactRef = db.collection('companies').doc('092').collection('contacts').doc(reminder.phoneNumber);
                const messagesRef = contactRef.collection('messages');
                await messagesRef.doc(sent.id._serialized).set(messageData, { merge: true });

                // Update reminder sent status
                await doc.ref.update({ reminderSentStatus: true });

                console.log(`Reminder sent for appointment at ${reminder.appointmentTime.toDate()}, to ${reminder.name}`);
            } catch (error) {
                console.error('Error sending scheduled reminder:', error);
            }
        }
    } catch (error) {
        console.error('Error retrieving scheduled reminders:', error);
    }
}

async function scheduleAppointmentReminderMessage(phoneNumber, name, appointment, timeFrame) {
    try {
        const appointmentTime = new Date(appointment.startTime);
        const reminderMessage = `Hi ${name},
            \nJust a quick reminder about our meeting scheduled for ${appointmentTime.toLocaleString()}.
            \n\nLooking forward to our discussion!
            \nIf you have any questions or need to reschedule, feel free to let me know.`;
        
        const formattedNumber = formatPhoneNumber(phoneNumber);
        const userRef = db.collection('user').doc('thealistmalaysia@gmail.com');
        const reminderCollectionRef = userRef.collection('scheduledAppointmentReminders');
        
        const scheduledTime = new Date(appointmentTime.getTime() - timeFrame);
        const docId = `${scheduledTime.getFullYear()}-${(scheduledTime.getMonth() + 1).toString().padStart(2, '0')}-${scheduledTime.getDate().toString().padStart(2, '0')} - ${name}`;
        
        const reminderData = {
            phoneNumber: formattedNumber,
            message: reminderMessage,
            scheduledTime: admin.firestore.Timestamp.fromDate(scheduledTime),
            appointmentTime: admin.firestore.Timestamp.fromDate(appointmentTime),
            reminderSentStatus: false,
            name: name
        };
        
        await reminderCollectionRef.doc(docId).set(reminderData);
        console.log(`${timeFrame / (60 * 60 * 1000)} hour(s) reminder scheduled and saved to Firebase for appointment: ${appointment.title}, Document ID: ${docId}`);
        return docId;
    } catch (error) {
...
async function scheduleAppointmentReminders(client) {
    try {
        ...
        const upcomingAppointments = await appointmentsCollectionRef
            .where('startTime', '>', now.toISOString())
            .where('startTime', '<', twoDaysFromNow.toISOString())
            .get();
        ...
        for (const doc of upcomingAppointments.docs) {
            ...
            // Check if the appointment has already been scheduled
            if (!appointment.reminderScheduled) {
                const startTime = new Date(appointment.startTime);
                const contact = appointment.contacts && appointment.contacts[0];
                
                if (contact) {
                    // Schedule 1 day before reminder
                    const oneDayBefore = new Date(startTime.getTime() - 24 * 60 * 60 * 1000);
                    if (oneDayBefore > now) {
                        await scheduleAppointmentReminderMessage(contact.id, contact.name, appointment, 24 * 60 * 60 * 1000);
                        scheduledCount++;
                    }
                    ...
                    if (twoHoursBefore > now) {
                        await scheduleAppointmentReminderMessage(contact.id, contact.name, appointment, 2 * 60 * 60 * 1000);
                        scheduledCount++;
                    }

                    // Mark the appointment as scheduled
                    await doc.ref.update({ reminderScheduled: true });
                }
            }
        }

        console.log(`Scheduled ${scheduledCount} reminders for ${upcomingAppointments.size} appointments`);
    } catch (error) {
...
async function handleNewMessagesAlist(client, msg, botName, phoneIndex) {
    ...
    // Initializing the scheduler for appointment reminders
    if (!isSchedulerInitialized) {
        isSchedulerInitialized = true;
        console.log('Initializing scheduler for appointment reminders...');
        
        // Schedule the reminders to run every minute
        schedule.scheduleJob('* * * * *', async function() {
            await sendScheduledAppointmentReminders(client);
        });
    }
    ...
    },
      {
        ...
        },
        {
            ...
            function: {
                ...
                description: "Check for available time slots in Google Calendar for the next specified number of days return back the name of date and time. only suggests slots that are 2 days after the current time.",                               parameters: {
                    type: "object",
                    parameters: {
                        type: "object",
                        properties: {
