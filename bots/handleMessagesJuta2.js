...
async function createCalendarEvent(summary, description, startDateTime, endDateTime, phoneNumber, contactName,companyName,idSubstring) {
    try {
        console.log('Creating calendar event with params:', { summary, description, startDateTime, endDateTime, phoneNumber, contactName,companyName });
        await addtagbookedFirebase(phoneNumber,'Booked Appointment', idSubstring);
        await addtagbookedFirebase(phoneNumber,'stop bot', idSubstring);
...
async function sendImage(client, phoneNumber, imageUrl, caption, idSubstring) {
  ...
  async function checkAndScheduleDailyReport(client, idSubstring) {
    try {
        const settingsRef = db.collection('companies').doc(idSubstring).collection('settings').doc('reporting');
        ...
        if (settings?.dailyReport?.enabled) {
            const reportTime = settings.dailyReport.time || "09:00";
            const groupId = settings.dailyReport.groupId;
            const lastRun = settings.dailyReport.lastRun?.toDate();
            ...
            if (wasRunToday) {
                console.log(`Daily report already sent for ${idSubstring} today`);
                return;
            }

            // Schedule the daily report using cron
            const job = cron.schedule(`${minutes} ${hours} * * *`, async () => {
                try {
                    // Double check it hasn't been sent yet by fetching fresh data
                    const currentSettings = (await settingsRef.get()).data();
                    const currentLastRun = currentSettings?.dailyReport?.lastRun?.toDate();
                    const currentNow = moment().tz('Asia/Kuala_Lumpur');
                    
                    if (currentLastRun && moment(currentLastRun).tz('Asia/Kuala_Lumpur').isSame(currentNow, 'day')) {
                        console.log(`Skipping duplicate daily report for ${idSubstring}`);
                        return;
                    }

                    // Try to atomically update lastRun to prevent duplicate sends
                    const updateSuccess = await db.runTransaction(async (transaction) => {
                        const settingsDoc = await transaction.get(settingsRef);
                        const currentData = settingsDoc.data();
                        const lastRunDate = currentData?.dailyReport?.lastRun?.toDate();
                        
                        if (lastRunDate && moment(lastRunDate).tz('Asia/Kuala_Lumpur').isSame(currentNow, 'day')) {
                            return false; // Already sent today
                        }

                        transaction.update(settingsRef, {
                            'dailyReport.lastRun': admin.firestore.Timestamp.now()
                        });
                        return true;
                    });

                    if (!updateSuccess) {
                        console.log(`Report already sent by another instance for ${idSubstring}`);
                        return;
                    }

                    // Get contacts added today
                    const { count, contacts } = await getContactsAddedToday(idSubstring);
                    
                    // Format the message
                    const message = `📊 *Daily Contact Report*\n\n` +
                    ...
                    if (groupId) {
                        ...
                        console.log(`Daily report sent to group ${groupId} at ${currentNow.format('HH:mm')}`);
                    }
                } catch (error) {
           ...
           // console.log(`Daily report scheduled for ${reportTime} (Asia/Kuala_Lumpur)`);
        }
    } catch (error) {
        console.error('Error setting up daily report schedule:', error);
    }
}
...
const DEFAULT_BUFFER_TIME = 30000; // 30 seconds
...
async function handleNewMessagesJuta2(client, msg, botName, phoneIndex) {
    ...
    // Get buffer time from Firebase
    let bufferTime = DEFAULT_BUFFER_TIME;
    try {
        const companyRef = db.collection('companies').doc(idSubstring);
        const companyDoc = await companyRef.get();
        if (companyDoc.exists) {
            // Use aiDelay from Firebase if it exists, otherwise use default
            bufferTime = (companyDoc.data().aiDelay || 30) * 1000; // Convert seconds to milliseconds
        }
    } catch (error) {
        console.error('Error fetching buffer time from Firebase:', error);
    }
    ...
    // Override buffer time for specific bot if needed
...
async function addtagbookedFirebase(contactID, tag, idSubstring) {
    ...
    try {
        await db.runTransaction(async (transaction) => {
            ...
            const templatesRef = db.collection('companies').doc(idSubstring).collection('followUpTemplates');
            const templatesSnapshot = await transaction.get(templatesRef);
            
            let matchingTemplate = null;
            templatesSnapshot.forEach(templateDoc => {
                const template = templateDoc.data();
                if (template.triggerTags?.includes(tag) && template.status === 'active') {
                    matchingTemplate = {
                        id: templateDoc.id,
                        ...template
                    };
            ...
            if (!currentTags.includes(tag)) {
                ...
                // If matching template found, trigger follow-up sequence
                if (matchingTemplate) {
                    try {
                        const response = await fetch('http://localhost:8443/api/tag/followup', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                            },
                            body: JSON.stringify({
                                requestType: 'startTemplate',
                                phone: contactID,
                                first_name: contactData.contactName || contactData.firstName || contactID,
                                phoneIndex: contactData.phoneIndex || 0,
                        ...
                        if (!response.ok) {
                            console.error(`Follow-up API error: ${response.statusText}`);
                        }
                    } catch (error) {
...
async function checkAvailableTimeSlots(daysAhead = 3) {
    const now = moment().tz('Asia/Kuala_Lumpur');
    const availableSlots = [];
    const slotDuration = 30 * 60 * 1000; // Fixed duration of 1 hour in milliseconds
    ...
    const startDate = now.clone().add(3, 'day').startOf('day');
    ...
    for (let dayOffset = 0; dayOffset < daysAhead; dayOffset++) {
        ...
        const endOfDay = dateToCheck.clone().set({hour: 21, minute: 0});
        ...
        for (let hour = 11; hour < 17; hour++) { // 14 is 2 PM, 16 is 4 PM (last slot starts at 4 PM)
        ...
        // Break the loop if we have found at least 3 available slots
...
async function sendDailyContactReport(client, idSubstring) {
    try {
        // Fetch reporting settings
        const settingsRef = db.collection('companies').doc(idSubstring).collection('settings').doc('reporting');
        const settings = (await settingsRef.get()).data();

        // If settings don't exist or reporting is disabled, return early
        if (!settings || !settings.dailyReport || !settings.dailyReport.enabled) {
            console.log('Daily reporting is disabled or not configured');
            return;
        }

        const { groupId, time } = settings.dailyReport;
        
        // Get current time in Asia/Kuala_Lumpur timezone
        const now = moment().tz('Asia/Kuala_Lumpur');
        const scheduledTime = moment.tz(time, 'HH:mm', 'Asia/Kuala_Lumpur');

        // If it's not the scheduled time, return
        if (now.format('HH:mm') !== scheduledTime.format('HH:mm')) {
            return;
        }

        // Get count of new contacts for today
...
async function waitForCompletion(threadId, runId, idSubstring, client, depth = 0,phoneNumber,name,companyName) {
    ...
    for (let attempts = 0; attempts < maxAttempts; attempts++) {
      try {
        ...
        if (runObject.status === 'completed') {
          ...
          const toolOutputs = await handleToolCalls(toolCalls, idSubstring, client,phoneNumber,name,companyName);
          console.log('Submitting tool outputs...');
          await openai.beta.threads.runs.submitToolOutputs(threadId, runId, { tool_outputs: toolOutputs });
          console.log('Tool outputs submitted, restarting wait for completion...');
          return await waitForCompletion(threadId, runId, idSubstring, client, depth + 1,phoneNumber,name,companyName);
...
async function runAssistant(assistantID, threadId, tools,idSubstring,client,phoneNumber,name,companyName) {
    ...
    const answer = await waitForCompletion(threadId, runId,idSubstring,client, 0,phoneNumber,name,companyName);
    return answer;
  }
...
async function handleToolCalls(toolCalls, idSubstring, client,phoneNumber,name,companyName) {
...
async function handlePDFMessage(msg, sender, threadID, client, idSubstring, extractedNumber) {
    try {
        ...
        // You might want to limit the text length to avoid token limits
        const truncatedText = pdfText.substring(0, 1000); // Adjust length as needed
        
        console.log('PDF text extracted:', truncatedText);
        return `[PDF Content Analysis: ${truncatedText}]`;
...
// Add this new function to handle message usage tracking
...
async function handleOpenAIAssistant(message, threadID, tags, phoneNumber, idSubstring, client,name) {
    ...
    const contactData = await getContactDataFromDatabaseByPhone(phoneNumber, idSubstring);
    if (tags !== undefined && tags.includes('demo')) {
    ...
    const answer = await runAssistant(assistantId, threadID, tools, idSubstring, client,phoneNumber,name,contactData.companyName);
    return answer;
}
async function analyzeAndSetUserProfile(phoneNumber, threadId, idSubstring,assistantId) {
    try {
        console.log('Analyzing chat history for user profiling...', phoneNumber);
        const chatHistory = await fetchRecentChatHistory(threadId);
        
        // Create a new thread for the profiling assistant to avoid mixing with main conversation
        const profilingThread = await openai.beta.threads.create();
        
        // Add the chat history to the profiling thread
        await openai.beta.threads.messages.create(profilingThread.id, {
            role: "user",
        ...
        const profileAnalysis = await waitForProfilingCompletion(profilingThread.id, run.id);
        
        // Store the raw analysis in Firebase
        await setUserProfile(idSubstring, phoneNumber, profileAnalysis);
        ...
        console.error('Error in analyzeAndSetUserProfile:', error);
        return JSON.stringify({ error: 'Internal process completed' });
    }
}
...
async function setUserProfile(idSubstring, phoneNumber, profileAnalysis) {
    ...
    try {
        ...
        console.log(`Profile updated for contact ${phoneNumber} in Firebase`);
    } catch (error) {
