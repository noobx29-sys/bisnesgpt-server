...
const MESSAGE_BUFFER_TIME = 60000; // 1 minute
const messageBuffers = new Map();
let processingLock = new Map(); // Add lock to prevent concurrent processing

async function handleNewMessagesDMAI(client, msg, botName, phoneIndex) {
    console.log('Handling new Messages '+botName);
    const chatId = msg.from;

    // Process immediate actions (Firebase, notifications)
    ...
    try {
         ...
         if (msg.hasMedia &&  (msg.type !== 'audio' || msg.type !== 'ptt')) {
           try {
               ...
               if (media) {
                if (msg.type === 'image') {
                    await client.sendMessage(msg.from, "Sorry, my phone memory is full, I can't download images. Please just message me.");
...
async function processBufferedMessages(client, chatId, botName, phoneIndex) {
    // Check if already processing messages for this chat
    ...
    try {
        ...
        // Get all messages currently in buffer
        const messages = [...buffer.messages];
        
        // Clear the processed messages from buffer
        buffer.messages = buffer.messages.slice(messages.length);
        ...
        // Release lock after processing or if error occurs
        processingLock.set(chatId, false);
        
        // If there are new messages in buffer, schedule another processing
        const remainingBuffer = messageBuffers.get(chatId);
        if (remainingBuffer && remainingBuffer.messages.length > 0) {
            remainingBuffer.timer = setTimeout(
                () => processBufferedMessages(client, chatId, botName, phoneIndex),
                MESSAGE_BUFFER_TIME
            );
        } else {
...
async function sendVoiceMessage(client, chatId, voiceUrl, caption = '') {
    try {
        ...
        // Validate file size
        if (fileSize > 16 * 1024 * 1024) { // 16MB limit
            throw new Error('File too large - exceeds 16MB WhatsApp limit');
        }

        // Create MessageMedia with detailed logging
        const media = new MessageMedia(
            'audio/mp4', // Use audio/mp4 for .m4a files
            Buffer.from(response.data).toString('base64'),
            `voice_${Date.now()}.m4a`
        );

        console.log('Created MessageMedia object:', {
            mimetype: media.mimetype,
            filename: media.filename,
            filesize: media.data.length
        });

        // Try to send with specific options
        const messageOptions = {
            sendAudioAsVoice: true,
            caption: caption || undefined
        };

        console.log('Sending with options:', messageOptions);
        ...
        while (attempts < maxAttempts) {
            try {
                ...
                lastError = err;
                attempts++;
                console.log(`Send attempt ${attempts} failed:`, err.message);
                
                if (attempts < maxAttempts) {
                    await new Promise(resolve => setTimeout(resolve, 1000));
                    // On retry, try without sendAudioAsVoice option
                    messageOptions.sendAudioAsVoice = false;
                }
            }
        }

        // If all attempts failed, throw error with details
        throw new Error(`Failed after ${maxAttempts} attempts: ${lastError.message}`);

    } catch (error) {
        console.error('Voice message send failed:', {
            error: error.message,
            url: voiceUrl,
            chatId: chatId,
            timestamp: new Date().toISOString()
...
try {
    ...
    const tempFile = path.join(os.tmpdir(), `temp_${Date.now()}.ogg`);
    await fs.promises.writeFile(tempFile, Buffer.from(audioData, 'base64'));
...
async function generateAudioResponse(text) {
    try {
        ...
        const buffer = Buffer.from(await mp3.arrayBuffer());
        const tempFile = path.join(os.tmpdir(), `speech_${Date.now()}.mp3`);
