...
async function sendDailyContactReport(client, idSubstring) {
  ...
  try {
    ...
    const dateStr = currentNow.format('YYYY-MM-DD');
    ...
    const coldLeadsQuery = await sqlClient.query(`
      SELECT
        ...
        EXTRACT(EPOCH FROM (NOW() - MAX(m.timestamp)))/3600 as hours_since_last_message
      FROM contacts c
    ...
    const message =
      ...
      (contacts.length > 0
        ...
        : "") +
      (coldLeads.length > 0
        ? `⚠️ *Leads Needing Follow-up (48h+):*\n${coldLeads
            .map((c) => `- ${c.contact_name || 'Unknown'} (${c.phone})`)
            .join("\n")}\n\n💡 *Tip:* Quick follow-ups can recover 30% of cold leads!\n\n`
        : "") +
