...
async function handleNewMessagesHajoon(client, msg, botName, phoneIndex) {
        ...
        switch (currentStep) {
            case steps.START:
                ...
                for (const template of followUpTemplates) {
                    if (template.keywordSource === "user" && template.triggerKeywords.some(kw => query.toLowerCase().includes(kw.toLowerCase()))) {
                        ...
                        if (response.keywordSource === "user" && response.keywords.some(kw => query.toLowerCase().includes(kw.toLowerCase()))) {
                            ...
                            for (let i = 0; i < response.videoUrls.length; i++) {
                                try {
                                    ...
                                    await addMessagetoFirebase(videoMessage, idSubstring, extractedNumber, contactName);
                        ...
                        if (response.keywordSource === "user" && response.keywords.some(kw => query.toLowerCase().includes(kw.toLowerCase()))) {
                            ...
                            for (let i = 0; i < response.documentUrls.length; i++) {
                                try {
                                    ...
                                    await addMessagetoFirebase(documentMessage, idSubstring, extractedNumber, contactName);
...
async function checkingStatus(threadId, runId) {
    ...
    if(status == 'completed') {
        try{
            ...
            console.log("error from handleNewMessagesHajoon: "+error)
            throw error;
        }
    }
...
async function runAssistant(assistantID, threadId, tools, idSubstring, client, phoneNumber, name) {
    ...
    var answer = await waitForCompletion(threadId, runId, idSubstring, client, 0, phoneNumber, name);
    return answer;
}
...
async function handleOpenAIAssistant(message, threadID, stopTag, extractedNumber, idSubstring, client, contactName) {
    ...
    const tools = [
        {
            type: "function",
            function: {
                name: "notifyDocumentSubmission",
                description: "Notify admin when a customer submits or mentions submitting documents",
                parameters: {
                    type: "object",
                    properties: {
                        details: {
                            type: "string",
                            description: "Details about the documents submitted or mentioned"
                        }
                    },
                    required: ["details"]
                }
    ...
    var answer = await runAssistant(assistantId, threadID, tools, idSubstring, client, extractedNumber, contactName);
    return answer;
}
async function handleToolCalls(toolCalls, idSubstring, client, phoneNumber, name) {
    ...
    for (const toolCall of toolCalls) {
        ...
        switch (toolCall.function.name) {
            case 'notifyDocumentSubmission':
                try {
                    console.log('Sending document submission notification...');
                    const args = JSON.parse(toolCall.function.arguments);
                    const result = await sendDocumentSubmissionNotification(client, args.details, name, phoneNumber, idSubstring);
                    toolOutputs.push({
                        tool_call_id: toolCall.id,
                        output: result,
                    });
                } catch (error) {
                    console.error('Error in handleToolCalls for notifyDocumentSubmission:', error);
                    toolOutputs.push({
                        tool_call_id: toolCall.id,
...
async function sendDocumentSubmissionNotification(client, details, customerName, customerPhone, idSubstring) {
    try {
        const notificationMessage = `*New Document Submission*\n\n` +
        ...
        const adminNumber = '601128968815@c.us';
        const sentMessage = await client.sendMessage(adminNumber, notificationMessage);
        await addMessagetoFirebase(sentMessage, idSubstring, '+601128968815');

        // Log document submission to Firebase if needed
        //await logDocumentSubmissionToFirebase(idSubstring, customerPhone, details);
...
module.exports = { handleNewMessagesHajoon };