...
async function scheduleFollowUpMessages(chatId, idSubstring, customerName, language, phoneIndex) {
    ...
    for (let day = 0; day < numberOfDays; day++) {
        ...
        for (let i = 0; i < messagesForDay.length; i++) {
            // Start 24 hours from now, with 1 second intervals
            const scheduledTime = moment().add(day + 1, 'days').add(i, 'seconds');
    ...
    // Schedule the staff reminder 1 second after the last message
    const lastDay = dailyMessages[numberOfDays - 1];
    const scheduledTime = moment().add(numberOfDays, 'days').add(lastDay.length, 'seconds');
